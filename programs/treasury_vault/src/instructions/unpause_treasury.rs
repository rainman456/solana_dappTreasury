...
/// Allow admin users to unpause the treasury
///
/// Accounts:
/// 0. `[writable]` treasury: The treasury account
/// 1. `[writable, signer]` authority: The user initiating the unpause
...
pub fn handler(
    ...
    // Check if treasury is already unpaused
    require!(treasury.is_paused, ErrorCode::TreasuryAlreadyUnpaused);
    ...
    emit!(TreasuryEvent {
        action: AuditAction::UnpauseTreasury as u8,
        treasury: treasury.key(),
